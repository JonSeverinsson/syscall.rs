...
def main():
    ...
    numbers32 = {
            ...
            'linux-x86_64': dict(load_table('arch/x86/entry/syscalls/syscall_64.tbl', {'common', 'x32'})),
            }
    ...
    for arch, nums in numbers32.items():
        if not nums:
            continue
        with open('../src/platform/{}/nr32.rs'.format(arch), 'w') as f:
            f.write('/* automatically generated by nr_from_src.py */\n\n')
            for name, nr in sorted(nums.items()):
                f.write('pub const {}: ::ureg = {};\n'.format(name.upper(), nr))
